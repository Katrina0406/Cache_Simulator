...
static void transpose_submit(size_t M, size_t N, double A[N][M], double B[M][N],
    ...
    if ((M == 32 && N == 32) || (M == 1024 && N == 1024)){
        for (i_c = 0; i_c < M; i_c += b) {
            for (j_c = 0; j_c < N; j_c += b) {
                for (size_t i = i_c; (i < i_c + b) && (i < M); i++) {
                    for (size_t j = j_c; (j < j_c + b) && (j < N); j++) {
                        if (i != j) {
                            B[j][i] = A[i][j];
                        }
                    }
                    if (i_c == j_c) {
                        // double v = A[i][i];
                        // B[i][i] = v;
                        tmp[(i + 8) % TMPCOUNT] = A[i][i];
                        B[i][i] = tmp[(i + 8) % TMPCOUNT];
                    }
                }
            }
        }
...
void registerFunctions(void) {
    ...
    registerTransFunction(trans_basic, "Transpose for slow matrix");
    registerTransFunction(trans_tmp, "Transpose for tmp replacement");
}
